...
// Auth:    Password login (bcrypt + session)
// Data:    Snowflake
// AI:      Anthropic → Groq fallback
// ─────────────────────────────────────────────────────────────────────────────
...
const loginLimiter = rateLimit({
  ...
  handler: function(req, res) { res.redirect('/login?error=lock'); }
});
...
app.post('/login', loginLimiter, async function(req, res) {
  ...
  if (!username || !password) return res.redirect('/login?error=bad');

  let users = [];
  try { users = JSON.parse(process.env.APP_USERS || '[]'); }
  catch (e) { return res.redirect('/login?error=bad'); }
...
function getSnowflakeConnection(callback) {
  if (sfConnection && sfConnection.isUp()) return callback(null, sfConnection);
  if (sfConnecting) { setTimeout(function() { getSnowflakeConnection(callback); }, 500); return; }
  sfConnecting = true;
  ...
  conn.connect(function(err, c) {
    sfConnecting = false;
    if (err) { console.error('Snowflake error:', err.message); return callback(err); }
    sfConnection = c;
...
app.post('/api/chat', requireAuth, aiLimiter, async function(req, res) {
  ...
  try {
    const aRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: model || 'claude-sonnet-4-20250514', max_tokens: max_tokens || 4000, system: system, messages: messages })
    });
    if (aRes.status === 429 || aRes.status === 529 || aRes.status === 503) throw new Error('RATE_LIMITED');
    if (!aRes.ok) { const t = await aRes.text(); throw new Error('ANTHROPIC_ERROR:' + aRes.status + ':' + t.slice(0, 100)); }
    const data = await aRes.json();
    data._provider = 'anthropic';
    return res.json(data);
  } catch (aErr) {
    const fallback = aErr.message === 'RATE_LIMITED' || aErr.message.startsWith('ANTHROPIC_ERROR:5');
    if (!fallback) return res.status(502).json({ error: 'AI error: ' + aErr.message });

    // Groq fallback
    try {
      const gMsgs = system ? [{ role: 'system', content: system }, ...messages] : [...messages];
      const gRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.GROQ_API_KEY },
        body: JSON.stringify({ model: 'llama-3.1-70b-versatile', max_tokens: max_tokens || 4000, messages: gMsgs, temperature: 0.3 })
      });
      if (!gRes.ok) { const t = await gRes.text(); console.error('Groq error:', gRes.status, t.slice(0, 200)); return res.status(502).json({ error: 'Both AI services unavailable.' }); }
      const gData = await gRes.json();
      return res.json({ content: [{ type: 'text', text: gData.choices[0].message.content }], stop_reason: gData.choices[0].finish_reason === 'length' ? 'max_tokens' : 'end_turn', _provider: 'groq', _fallback: true });
    } catch (gErr) {
      return res.status(502).json({ error: 'All AI services unavailable.' });
    }
  }
});
...
app.listen(PORT, function() {
  console.log('FLIK Survey Intelligence on port ' + PORT + ' [' + process.env.NODE_ENV + ']');
});
